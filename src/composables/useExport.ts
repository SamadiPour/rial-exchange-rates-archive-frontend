...
export function useExport() {
  ...
  const exportAsExcel = (
    ...
    const header = [
      'Date',
      ...currencies.flatMap((currency) => [
        `${currency.toUpperCase()} Buy`,
        `${currency.toUpperCase()} Sell`,
      ]),
    ];
    const rows = dates.map((date) => {
      const row = [date];
    ...
    try {
      ...
      canvas.toBlob((blob) => {
        if (blob) {
          const name =
            filename ||
            `rial-exchange-chart-${format(new Date(), 'yyyy-MM-dd-HHmm')}.png`;
  ...
  const exportChartAsSVG = async (
    ...
    try {
      ...
      const name =
        filename ||
        `rial-exchange-chart-${format(new Date(), 'yyyy-MM-dd-HHmm')}.svg`;
  ...
  const exportChartAsPDF = async (
    ...
    try {
      ...
      const name =
        filename ||
        `rial-exchange-chart-${format(new Date(), 'yyyy-MM-dd-HHmm')}.pdf`;
      pdf.save(name);
    } catch (error) {
