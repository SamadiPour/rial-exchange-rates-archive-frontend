...
export function useUrlState() {
  ...
  const updateUrl = () => {
    ...
    updateTimeout = setTimeout(() => {
      ...
      if (selectedDateRange.value.start && selectedDateRange.value.end) {
        params.set(
          'startDate',
          format(selectedDateRange.value.start, 'yyyy-MM-dd'),
        );
        params.set(
          'endDate',
          format(selectedDateRange.value.end, 'yyyy-MM-dd'),
        );
      }
  ...
  const startWatching = () => {
    watch([selectedCurrencies, selectedDateRange, roiEnabled], updateUrl, {
      deep: true,
    });
  };
